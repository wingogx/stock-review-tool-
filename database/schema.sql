...
CREATE TABLE limit_stocks_detail (
    ...
    sealed_amount DECIMAL(20, 2), -- 封单金额（买一的总金额）
...
CREATE TABLE dragon_tiger_board (
    ...
    lhb_buy_amount DECIMAL(20, 2),
    lhb_sell_amount DECIMAL(20, 2),
    lhb_net_amount DECIMAL(20, 2),
    reason TEXT,
    created_at TIMESTAMP DEFAULT NOW(),
...
CREATE TABLE watchlist_monitoring (
    ...
    abnormal_reason TEXT,  -- 异动原因（大涨、大跌、放量等）
...
-- 自动计算市场情绪数据的触发器函数（示例）
CREATE OR REPLACE FUNCTION update_market_sentiment()
RETURNS TRIGGER AS $$
BEGIN
    -- 这里可以添加自动计算逻辑
    RETURN NEW;
